...
from helpers import load_data, one_hot_encode, standardize, get_interaction_terms_columns, add_interaction_terms_columns
...
if __name__ == '__main__':
    ...
    print('\nWe have found', len(co_linear_feature_columns), 'interaction terms with at least that correlation')
    X_train_public = add_interaction_terms_columns(X_train_public, co_linear_feature_columns)
    X_test_public = add_interaction_terms_columns(X_test_public, co_linear_feature_columns)

    # Training model
    print('\nTraining model...')
    print('Note: this might take a bit, since we are training on a (250000, 250+)-sized dataset')
    max_iterations = 1000
    gamma = 0.1
    w, loss = reg_logistic_regression_AGDR(Y_train_public, X_train_public, lambda_=0,
